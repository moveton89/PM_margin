...
function telegramUser() {
  ...
  if (!u) return { id: 'demo', name: 'Пользователь' };
...
function apiHeaders() {
  ...
  return {
    ...
    'X-Telegram-User-Id': u.id,
    'X-Telegram-User-Name': encodeURIComponent(u.name)
  };
}
...
function applyConfig() {
  ...
  if (c.colors) {
    ...
    if (c.colors.bg) root.style.setProperty("--bg", c.colors.bg);
    if (c.colors.card) root.style.setProperty("--card", c.colors.card);
    if (c.colors.text) root.style.setProperty("--text", c.colors.text);
  }
  document.getElementById("versionLabel").textContent = c.versionLabel || "Telegram Mini App";
  document.getElementById("appTitle").textContent = c.appTitle || "Калькулятор маржи";
  document.getElementById("appSubtitle").textContent = c.subtitle || "";
  document.getElementById("historyBtn").textContent = c.historyButton || "История";
  document.getElementById("resetBtn").textContent = c.resetButton || "Сбросить";
  document.getElementById("saveCalcBtn").textContent = c.saveButton || "Сохранить расчет";
  document.getElementById("historyVersionLabel").textContent = c.versionLabel || "Версия";
  document.getElementById("historyTitle").textContent = c.historyTitle || "История расчетов";
  document.getElementById("closeHistoryBtn").textContent = c.closeButton || "Закрыть";
  for (const [key, override] of Object.entries(c.scenarios || {})) {
    if (!scenarios[key]) continue;
    scenarios[key] = { ...scenarios[key], ...override };
  }
}
...
function renderForm() {
  ...
  form.innerHTML = `<div class="group-title"></div>`;
  state[active] ||= {};
...
function currentInputs() {
  return Object.fromEntries(scenarios[active].fields.map(([key, label, def]) => [key, { label, value: val(key, def) }]));
}
function currentKpisForSave() {
  return (lastResult?.kpis || []).map(([label, value, format]) => ({ label, value, format }));
}
  ...
  try {
    const res = await fetch('/api/calculations', {
      ...
      body: JSON.stringify({
        ...
        name: sc.tab + ' — ' + new Date().toLocaleString('ru-RU'),
        inputs: currentInputs(),
        kpis: currentKpisForSave(),
        summaryHtml: details.innerHTML
      })
    });
    if (!res.ok) throw new Error('save failed');
    saveStatus.textContent = 'Готово: расчет сохранен';
...
function formatSavedValue(k) {
  if (k.format === 'rub') return rub.format(k.value) + ' ₽';
  if (k.format === 'pct') return pct.format(k.value);
  return String(k.value);
}
async function loadHistory() {
  historyPanel.classList.remove('hidden');
  historyList.innerHTML = '<div class="notice">Загружаю историю...</div>';
  try {
    const res = await fetch('/api/calculations', { headers: apiHeaders() });
    const data = await res.json();
    const items = data.calculations || [];
    if (!items.length) {
      historyList.innerHTML = '<div class="notice">История пока пустая. Сохраните первый расчет.</div>';
      return;
    }
    historyList.innerHTML = items.map(item => {
      const kpisHtml = item.kpis.map(k => '<div class="mini-kpi"><span>' + k.label + '</span><strong>' + formatSavedValue(k) + '</strong></div>').join('');
      return '<article class="history-item" data-id="' + item.id + '"><div class="history-item-head"><div><strong>' + item.name + '</strong><small>' + new Date(item.createdAt).toLocaleString('ru-RU') + '</small></div><button class="danger-link" data-delete="' + item.id + '">Удалить</button></div><div class="mini-kpis">' + kpisHtml + '</div></article>';
    }).join('');
    historyList.querySelectorAll('[data-delete]').forEach(btn => {
      btn.onclick = async () => {
        await fetch('/api/calculations/' + btn.dataset.delete, { method: 'DELETE', headers: apiHeaders() });
        loadHistory();
      };
    });
  } catch (e) {
    historyList.innerHTML = '<div class="notice">Историю загрузить не удалось.</div>';
  }
}
...
// PlantMarket 3.0 Premium additions
function pmOpenPdfReport() {
  document.title = 'PlantMarket — отчет';
  window.print();
}
function pmOpenLead() { document.getElementById('leadPanel')?.classList.remove('hidden'); }
function pmCloseLead() { document.getElementById('leadPanel')?.classList.add('hidden'); }
async function pmSendLead() {
  const status = document.getElementById('leadStatus');
  if (status) status.textContent = 'Отправляю...';
  try {
    const res = await fetch('/api/leads', { method: 'POST', headers: apiHeaders(), body: JSON.stringify({ phone: document.getElementById('leadPhone')?.value || '', comment: document.getElementById('leadComment')?.value || '', user: telegramUser() }) });
    if (!res.ok) throw new Error('lead failed');
    if (status) status.textContent = 'Заявка отправлена';
    tg?.HapticFeedback?.notificationOccurred?.('success');
  } catch(e) { if (status) status.textContent = 'Не удалось отправить заявку'; }
}
setTimeout(() => {
  document.getElementById('pdfBtn')?.addEventListener('click', pmOpenPdfReport);
  document.getElementById('leadOpenBtn')?.addEventListener('click', pmOpenLead);
  document.getElementById('closeLeadBtn')?.addEventListener('click', pmCloseLead);
  document.getElementById('leadBackdrop')?.addEventListener('click', pmCloseLead);
  document.getElementById('sendLeadBtn')?.addEventListener('click', pmSendLead);
  const titleEl = document.getElementById('appTitle'); if (titleEl && titleEl.textContent === 'Калькулятор маржи') titleEl.textContent = 'PlantMarket';
  const v = document.getElementById('versionLabel'); if (v) v.textContent = 'PlantMarket · версия 3.0 Premium';
}, 300);
