...
## Версия 2.0 — сохранение расчетов

Добавлено:
- кнопка **Сохранить расчет**;
- экран **История**;
- хранение расчетов по Telegram-пользователю;
- удаление сохраненных расчетов.
...
Для Render Free без диска данные могут пропасть после пересборки/перезапуска. Для постоянного хранения подключите Render Disk и добавьте переменную:

```env
DATA_FILE=/var/data/calculations.json
```
